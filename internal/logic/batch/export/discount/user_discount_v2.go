...
type TaskUserDiscountV2Export struct {
}

func (t TaskUserDiscountV2Export) TaskName() string {
	return "UserDiscountExport"
}

func (t TaskUserDiscountV2Export) Header() interface{} {
	return ExportUserDiscountEntity{}
}

func (t TaskUserDiscountV2Export) PageData(ctx context.Context, page int, count int, task *entity.MerchantBatchTask) ([]interface{}, error) {
	...
	req := &userDiscountListInternalReqV2{
	...
	req.SkipTotal = true
	result := merchantUserDiscountCodeListV2(ctx, req)
	if result != nil {
		// Preload all related data
		preload := preloadUserDiscountData(ctx, result)
...
// UserDiscountPreloadData holds all preloaded data for user discounts
type UserDiscountPreloadData struct {
	Users map[uint64]*bean.UserAccount
	Plans map[uint64]*bean.Plan
}

// preloadUserDiscountData preloads all related data for user discounts in bulk
func preloadUserDiscountData(ctx context.Context, userDiscounts []*userDiscountDetailV2) *UserDiscountPreloadData {
	preload := &UserDiscountPreloadData{
...
type userDiscountListInternalReqV2 struct {
...
type userDiscountDetailV2 struct {
	Id             int64             `json:"id"`
...
func merchantUserDiscountCodeListV2(ctx context.Context, req *userDiscountListInternalReqV2) []*userDiscountDetailV2 {
	...
	var resultList []*userDiscountDetailV2
	for _, item := range mainList {
		resultList = append(resultList, &userDiscountDetailV2{
			Id:             item.Id,
