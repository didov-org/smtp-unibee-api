...
func GetMerchantExchangeCurrencyRate(ctx context.Context, merchantId uint64, fromCurrency string, toCurrency string) (exchangeRate float64) {
	fromCurrency = strings.ToUpper(fromCurrency)
	toCurrency = strings.ToUpper(toCurrency)
	if fromCurrency == toCurrency {
		return 1
	}
	exchangeApiKeyConfig := merchant_config.GetMerchantConfig(ctx, merchantId, FiatExchangeApiKey)
	var rate *float64
	...
	if exchangeApiKeyConfig != nil && len(exchangeApiKeyConfig.ConfigValue) > 0 {
		rate, err = GetExchangeConversionRates(ctx, exchangeApiKeyConfig.ConfigValue, fromCurrency, toCurrency)
	} else {
		rate, err = GetExchangeConversionRateFromClusterCloud(ctx, fromCurrency, toCurrency)
	}
	utility.AssertError(err, "exchange currency error")
	utility.Assert(rate != nil, "exchange currency error, rate is nil")
	return *rate
}
