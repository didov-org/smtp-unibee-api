...
func SetupForCloudMode(ctx context.Context, merchantId uint64) error {
	if config.GetConfigInstance().Mode == "cloud" && !config.GetConfigInstance().IsProd() {
		...
		//}
		//{
		//	stripeGateway := query.GetDefaultGatewayByGatewayName(ctx, consts.CloudModeManagerMerchantId, "stripe")
		//	if stripeGateway != nil {
		//		service.SetupGateway(ctx, merchantId, stripeGateway.GatewayName, stripeGateway.GatewayKey, stripeGateway.GatewaySecret, stripeGateway.SubGateway, nil, nil, nil, nil, nil)
		//	}
		//}
	}
	return nil
