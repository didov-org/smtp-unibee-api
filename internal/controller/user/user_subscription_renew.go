...
func (c *ControllerSubscription) Renew(ctx context.Context, req *subscription.RenewReq) (res *subscription.RenewRes, err error) {
	...
	if len(req.SubscriptionId) == 0 {
		one := query.GetLatestActiveOrIncompleteSubscriptionByUserId(ctx, _interface.Context().Get(ctx).User.Id, _interface.GetMerchantId(ctx), req.ProductId)
		if one != nil {
			req.SubscriptionId = one.SubscriptionId
		} else {
			one = query.GetLatestSubscriptionByUserId(ctx, _interface.Context().Get(ctx).User.Id, _interface.GetMerchantId(ctx), req.ProductId)
